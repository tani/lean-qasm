...
syntax (name := qasmProgramCommand) "qasm%" ident "{" qasmBlock : command
syntax (name := qasmFileProgramCommand)
  "qasmFile%" ident str ("using" term)? : command
```
...
```lean
@[command_elab qasmProgramCommand]
meta unsafe def elaborateQasmProgram : CommandElab
  | stx =>
      compileProgram stx[1]!.getId.toString "<qasm%>" stx[3]!.getAtomVal {}

@[command_elab qasmSourceProgramCommand]
meta unsafe def elaborateQasmSourceProgram : CommandElab
  | `(command| qasm% $name:ident from $source:term $[using $options?]?) => do
      let sourceValue ← evalSource source
      let optionsValue ← evalOptions options?
      compileProgram name.getId.toString "<qasm%>" sourceValue optionsValue
  | _ => throwUnsupportedSyntax

@[command_elab qasmFileProgramCommand]
meta unsafe def elaborateQasmFileProgram : CommandElab
  | `(command| qasmFile% $name:ident $path:str $[using $options?]?) => do
      ...
      let optionsValue ← evalOptions options?
      compileProgram name.getId.toString (toString resolved) source optionsValue
  | _ => throwUnsupportedSyntax
end Compiler
end QASM
```
