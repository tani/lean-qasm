...
- Compile changed files that are outside Lake targets with `lake env lean <file>`. Current standalone roots include `Example.lean`, `LoweringCommands.lean`, `LoweringRegression.lean`, and `Scratch.lean`; smoke or scratch modules not imported by `QASM.lean` must also be checked directly.
