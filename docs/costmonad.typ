...
= QASM の静的メトリクスでは StateM を使う

Lean QASM では、コストは実行結果に添えるのではなく、走査中の `Metrics` 状態として集める。
...
ここでの `Metrics` はゲート宣言、操作、分岐、ループなどの個数を持つレコードである。
`charge` は値を返さず、現在の集計値だけを更新する。
...
partial def costCircuit : Circuit -> CostM Unit
  ...
  | .primitive _          => charge { operations := 1 }
```
...
同じ `bind` / `do` の形を保ちつつ、`Nat` を `Metrics` や重みつきコストへ交換できる。
...
#block(fill: rgb("#eef2ff"), inset: 0.9em, radius: 5pt)[
  ...
  4. QASM では同じ発想を `StateM Metrics` と canonical IR の走査に使う。
]
